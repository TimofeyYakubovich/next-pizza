...
export const SearchInput: React.FC<Props> = ({ className }) => {
    ...
    useClickAway(ref, () => { // передаем в useClickAway ссылку ref и при нажатии на ref отрабатывает колбек
    ...
    useDebounce( // замена useEffect
        ...
        250, // каждые 250 милисекунд будет проверка если searchQuery изменился будет запрос если нет то запроса не будет
        [searchQuery],
    );
    ...
    return (
        <>
            ...
            <div ref={ref} className={cn('flex rounded-2xl flex-1 justify-between relative h-11 z-30', className)}>
                ...
                {/* попап рендерит список найденых продуктов */}
                {products.length > 0 && (
                    <div
                        className={cn(
